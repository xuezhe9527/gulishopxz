...
export default {
  ...
  methods: {
    ...
    async pay() {
      ...
      //使用element-ui弹出对话框
      try {
        this.$alert(`<img src="${imgUrl}"/>`, "请用微信扫码支付", {
          ...
          beforeClose: (action, instance, done) => {
            if (action === "confirm") {
              if (this.status === 200) {
                ...
                this.$message.warning('请确认您已经支付过 了')
              }
            } else if (action === "cancel") {
              clearInterval(this.timer)
              this.timer =null
              this.$message.warning("请联系我们，电话110")
              done()
            }
          },
        })
      ...
      //设置一个定时器，用来监测用户订单是否成功支付
      if (!this.timer) {
        this.timer = setInterval(async () => {
          const result = await this.$API.reqPayStatus(this.orderInfo.orderId);
          if (result.code === 200) {
            // 为了保证在未扫码的情况下，用户不能直接跳转至支付成功页面，需要用一个全局变量来保存知否成功支付的结果
            this.status = result.data;
            alert("支付成功了，跳转至支付成功页面");
            clearInterval(this.timer);
            this.timer = null;
            this.$router.push("/paysuccess");
            this.$msgbox.close();
